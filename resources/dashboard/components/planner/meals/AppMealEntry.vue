<template>
    <div class="food-block food-block__total">
        ...
        <div class="food-block__main">
            ...
            <img :src="item.product.image ? item.product.image : '/images/no-product-image.jpg'" onerror="this.src='/images/no-product-image.jpg?t=0'" class="food-block__group-img">

            <div class="food-block__text">
                <div class="food-block__text-top">
                    <span class="food-block__name food-block__name--height-40"><a v-if="item.product.url" :href="item.product.url" target="_blank" class="food__links food__links-solo">
            ...
            <div class="food-block__info food-block__info-one">
                <div class="food-block__show-group food-block__show-group--one">
                    <span class="text">{{(item.product.calories * item.servings)}} cal.</span>
                </div>
            </div>

            <div class="radio">
                <input-switch data-v-step="2" v-model="item.is_eaten" v-on:change="handleItemUpdate" />
            </div>
        ...
        >
            <div class="food-block__show-group food-block__show-group--four">
                ...
                <span class="text">{{item.product.store.name}}</span>
            </div>

            <div class="food-block__show-group food-block__show-group--four">
                <span class="title">Serving Size</span>
                <span class="text" v-tooltip="item.product.package_size">{{item.product.serving_size}}</span>
            </div>

            <div class="food-block__show-group food-block__show-group--four">
                <span class="title">Servings</span>
                <span class="text">{{item.servings}}</span>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async handleItemUpdate(checked) {
                this.is_busy = true;
                try {
                    let res = await this.$http.patch(`/api/meal/eaten/${this.item.id}`, {
                        is_eaten: checked
                    })

                    if(res.data.success) {
                        this.$emit('updateitemeaten', {
                            ...this.item,
                            is_eaten: checked
                        })
