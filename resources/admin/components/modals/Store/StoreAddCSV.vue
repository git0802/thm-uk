<template>
    <v-form ref="form" v-model="valid">
        <v-row class="xs12">
            ...
            <v-col class="d-flex" cols="12" sm="6">
                <v-btn :disabled="!isValidated || !!dupes.length || uploading" :loading="uploading" @click="handleStore">
                    Store
                </v-btn>
    ...
    methods: {
        ...
        async fetchShops() {
            try {
                ...
                this.$store.commit('stores/refreshStoresList', res.data.data)
            } catch (e) {}
        },
        async handleStore() {
            this.uploading = true;
            ...
            data.set('original_url', this.inputs.url.value);
